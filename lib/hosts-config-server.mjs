...
// Ported from lib/hosts-config.ts to keep the .mjs twin at parity.
let storedSelfAliasesCache = []

function loadStoredSelfAliases() {
  try {
    if (!fs.existsSync(HOSTS_CONFIG_PATH)) { storedSelfAliasesCache = []; return }
    const config = JSON.parse(fs.readFileSync(HOSTS_CONFIG_PATH, 'utf-8'))
...
export function isSelf(hostId) {
  ...
  // resolve to this machine. Catches stale hostnames after hostname changes.
  // cachedHosts guard prevents recursion during loadHostsConfig().
  if (cachedHosts !== null) {
...
function validateHosts(hosts) {
  ...
  // Ensure self host exists (among enabled hosts) — use isSelfHost() which
  // checks URL/aliases too, so we can detect ourselves even after a hostname
  // change. Only enabled hosts are eligible as the canonical self entry.
  const selfIdx = validHosts.findIndex(host => isSelfHost(host) && host.enabled !== false)
  if (selfIdx === -1) {
...
export function clearHostsCache() {
  ...
  // Reload the stored self-alias cache from disk so isSelf() stays accurate
  // after a hosts.json rewrite (e.g. hostname-migration persist).
  loadStoredSelfAliases()
}
 ...
 *
 * Ported from the saveHosts() function in lib/hosts-config.ts (the
 * TypeScript twin of this file) so that the auto-hostname-migration
 * path in validateHosts() can persist its changes. Preserves non-hosts
 * fields (organization, organizationSetAt, organizationSetBy) by
 * reading the existing config before writing.
 *
 * @param {Array} hosts - Array of host objects to persist
...
function saveHostsToFile(hosts) {
  try {
    ...
    if (fs.existsSync(HOSTS_CONFIG_PATH)) {
      try {
        existing = JSON.parse(fs.readFileSync(HOSTS_CONFIG_PATH, 'utf-8'))
      } catch {
        console.warn('[Hosts] Could not parse existing hosts.json, preserving only hosts array')
      }
    }

    const config = { ...existing, hosts }
