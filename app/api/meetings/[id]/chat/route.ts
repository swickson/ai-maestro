...
export async function POST(
  ...
  if (meeting) {
    ...
    try {
      const { appendChatMessage } = await import('@/lib/meeting-chat')
      const chatMessage = appendChatMessage(meetingId, {
        from: body.from as string,
        fromAlias: body.fromAlias as string | undefined,
        fromLabel: body.fromLabel as string | undefined,
        fromType: (body.fromType as string) || 'agent',
...
export async function GET(
  ...
  if (meeting) {
    try {
      const { getChatMessages } = await import('@/lib/meeting-chat')
      const messages = getChatMessages(meetingId, since || undefined)
