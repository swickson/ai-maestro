...
# lets a new team (N4 today, AllianceOS next) reuse it with zero image change.
# See docs/CLOUD-AGENT-DB-ISOLATION.md (§3, §3.4). Owner: ops (Hutch).
#
...
#
# NOTE: deliberately NOT `set -u` — the sourced Holmes shell snapshot trips
...
# The ziggy MCP memory pool falls back to DATABASE_URL when MEMORY_DATABASE_URL
...
# MEMORY_DATABASE_URL would silently route ziggy memory writes into the WRONG
...
# extraEnv) so it never fires for workers or for other ziggy agents that
# correctly rely on the DATABASE_URL fallback (their DATABASE_URL already IS the
# ziggy memory DB). Independent of the PG-bootstrap gate below.
if [ "${REQUIRE_MEMORY_DATABASE_URL:-0}" = "1" ] && [ -z "${MEMORY_DATABASE_URL:-}" ]; then
  ...
  printf '[db-isolation]   ziggy memory would fall back to DATABASE_URL (the app/test DB) and write there.\n' >&2
...
# loopback (Zach's change), so this is what makes the suite hit loopback even
