...
> **As-deployed reconciliation (2026-06-20, ops/the ops agent — verified against the LIVE a dev team containers (an orchestrator agent + an agent worker) and the allocator code, not re-derived from this doc):** the wave arc has **SHIPPED**; several per-section tags below predate the rollout and are corrected inline. **Now [LANDED] and running:** §11.1 three-path/two-profile layout, §11.2 `/ai-team` as its own RO-workers / RW-orchestrator repo, §11.4 host-local bare-repo transport, §11.6 per-agent git identity, §11.7 the port allocator. **Genuinely still [TO-BUILD]:** the §11.4 *cross-host* transport variant (Ziggy ran single-host) and §11.5 automated drift-management (no live evidence the event-driven `main`→wave merge is wired). Live anchors for a mirror deploy: identity = per-agent `user.name` + shared `user.email = operator@example.com` (mounted RO at `/home/claude/.gitconfig`); transport = `~/.aimaestro/transport/<team>/<wave>.git` → `/transport.git`; allocator = lockfile mutex `~/.aimaestro/.cloud-port-alloc.lock` + registry-as-reservation.
...
- **[LANDED]** Cloud dev-team containers commit with **per-agent `user.name` + a shared deploy-sanctioned `user.email`**, provisioned once into the per-agent `/home/claude/.gitconfig` (RO). Verified on the live a dev team: each agent→its own `name`, both `email=operator@example.com` — attribution (the per-agent NAME) is real today. This **supersedes** the earlier generic `AI Maestro Agent <agent@example.com>` default. If a repo's deploy tooling ever keys on name too, fall back to author = the shared identity + `Co-authored-by:` trailers (the safe superset).
...
**The fix [LANDED]:** a per-host lock around the **existing registry** (NOT a new sqlite/reservations file — a second store re-creates the two-sources-disagree problem that *is* the status-lies defect). Implemented in `agents-docker-service.ts` as a **stale-reclaiming lockfile mutex** at `~/.aimaestro/.cloud-port-alloc.lock` (Node has no `flock()` binding and the `flock` binary is absent on macOS, so a lockfile rather than the literal `flock`), with `computeReservedCloudPorts` (registry union incl hibernated + soft-deleted ∪ `ss`/docker host-bound backstop) and `pickFirstFreeCloudPort` (fail-loud on exhaustion); range configurable via `CLOUD_AGENT_PORT_RANGE_START/END` (default 23001-23100). Critical section: take per-host lock → read registry → `reserved = {all agents' recorded ports} ∪ {bound host ports backstop}` → pick first free → **persist the reservation, THEN `docker run`** (a failed bind reuses the same reserved port on retry — idempotent, no leak) → release lock.
...
- **Sweeping an intentionally-hibernated agent is safe — because the container starts but the agent does not act.** `update-runtime` rebuilds **and starts** the container (there is **no** non-starting reassignment path: a plain `wake` is just `docker start` of the existing container on its **old baked port**, so patching the registry port *without* `update-runtime` would **desync** — registry says the new port, the container is still bound to the old one. Only the `update-runtime` rebuild re-bakes the new port, and it always brings the process up). BUT it does **NOT** fire the **on-wake hook** (the hook fires on the `wake` route only — §6). So the agent comes up **idle and unprimed**: with no on-wake prompt injected it does **not** tear off on stale instructions, and any queued AMPs simply sit in its inbox until an **intentional** `wake` fires the hook. Net: an `update-runtime` on a hibernated agent **parks it safely** on its new port and it is woken deliberately when its next role opens — the port-sweep restarts the container without disturbing a parked agent's behavior. Heads-up the owner first (identity is preserved, but the port moves).

### 11.9 Workstreams & status
1. **Port-reservation** — lockfile-mutex + registry-as-reservation, fail-loud, configurable range, parallel-recreate race folded in. **[LANDED]** (`agents-docker-service.ts`; was the GATING prereq — now shipped, default range 23001-23100).
2. **Cloud-migrate dev agents** — the §11.1 profiles + the §11.4 bare-repo transport + the §11.3 wave loop (host-side volume/transport + container-profile provisioning in `agents-docker-service`). **[LANDED]** on the a dev team (an orchestrator agent + multiple worker agents workers, host-local transport). Cross-host variant still **[TO-BUILD]**.
3. **Instruction relocation + on-wake reinforcement** — `$HOME` global-path placement (`~/.codex/AGENTS.md`, `~/.gemini/GEMINI.md`) + per-agent `user.name` git identity + the reinforcement hook. Per-agent `user.name` git identity **[LANDED]** (§11.6); the per-agent instruction mount (#191, `instructions.md` → `/home/claude/.claude/CLAUDE.md`) is in place. `$HOME` global `AGENTS.md`/`GEMINI.md` placement + the reinforcement hook **[TO-BUILD]** (unverified by ops).
