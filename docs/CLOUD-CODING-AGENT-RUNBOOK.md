...
> **As-deployed reconciliation (2026-06-20, ops/Hutch — verified against the LIVE Ziggy dev-team containers (Sam orchestrator + Al worker) and the allocator code, not re-derived from this doc):** the wave arc has **SHIPPED**; several per-section tags below predate the rollout and are corrected inline. **Now [LANDED] and running:** §11.1 three-path/two-profile layout, §11.2 `/ai-team` as its own RO-workers / RW-orchestrator repo, §11.4 host-local bare-repo transport, §11.6 per-agent git identity, §11.7 the port allocator. **Genuinely still [TO-BUILD]:** the §11.4 *cross-host* transport variant (Ziggy ran single-host) and §11.5 automated drift-management (no live evidence the event-driven `main`→wave merge is wired). Live anchors for a mirror deploy: identity = per-agent `user.name` + shared `user.email = swickson@todoverde.com` (mounted RO at `/home/claude/.gitconfig`); transport = `~/.aimaestro/transport/<team>/<wave>.git` → `/transport.git`; allocator = lockfile mutex `~/.aimaestro/.cloud-port-alloc.lock` + registry-as-reservation.
...
- **[LANDED]** Cloud dev-team containers commit with **per-agent `user.name` + a shared deploy-sanctioned `user.email`**, provisioned once into the per-agent `/home/claude/.gitconfig` (RO). Verified on the live Ziggy team: Sam→`name=Sam`, Al→`name=Al`, both `email=swickson@todoverde.com` — attribution (the per-agent NAME) is real today. This **supersedes** the earlier generic `AI Maestro Agent <agent@example.com>` default. If a repo's deploy tooling ever keys on name too, fall back to author = the shared identity + `Co-authored-by:` trailers (the safe superset).
...
2. **Cloud-migrate dev agents** — the §11.1 profiles + the §11.4 bare-repo transport + the §11.3 wave loop (host-side volume/transport + container-profile provisioning in `agents-docker-service`). **[LANDED]** on the Ziggy team (Sam orchestrator + Al/Gushie/Tina workers, host-local transport). Cross-host variant still **[TO-BUILD]**.
